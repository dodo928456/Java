...
public class CalculationServerFrame extends JFrame {
	...
	public CalculationServerFrame(){
		...
		c.add(new JLabel("계산서버 입니다"));
		c.add(new JScrollPane(log),BorderLayout.CENTER);
		setVisible(true);
		
		new ServerThread().start();//서비스시작
	}
	...
	class ServiceThread extends Thread{
		...
		public ServiceThread(Socket socket){//클라이언트 통신할소캣을 전달받음
		...
		public void run(){
			while(true){
				try{
					String first = in.readLine();//첫수
					String operator = in.readLine();//첫수
					String second = in.readLine();//둥째수
